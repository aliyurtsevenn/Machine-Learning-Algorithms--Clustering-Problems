...
metric_type=data[data.columns[0]][8]
...
# Create the model
dbscan_model= DBSCAN(eps=epsilion_value, min_samples=min_cluster_num, metric=metric_type)
...
try:
    ...
    root = tk.Tk()
    root.title("Results")
    T = tk.Text(root, height=6, width=60,font=("Helvetica", 14))
    T.pack()
